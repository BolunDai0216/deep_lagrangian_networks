...
class DeepLagrangianNetwork(tf.keras.Model):
    ...
    def call(self, state, velocity, tau, training=True):
        ...
        # Computer ddq_pred
        ddq_pred = tf.linalg.inv(M) @ ((tau - C - G)[:, :, tf.newaxis])
        ddq_pred = ddq_pred[:, :, 0]

        return ddq_pred, M, C, G
...
def main():
    ...
    tau = tf.ones([64, n_dof])

    delan_model(state, velocity, tau)
