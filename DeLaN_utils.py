...
def plot_test2(
    ...
    # Plot the performance:
    y_t_low = np.clip(
        1.2 * np.min(np.vstack((test_ddq, delan_ddq)), axis=0), -np.inf, -0.01
    )
    y_t_max = np.clip(
        1.5 * np.max(np.vstack((test_ddq, delan_ddq)), axis=0), 0.01, np.inf
    )
    ...
    # Plot Acceleration
    ax0 = fig.add_subplot(2, 4, 1)
    ax0.set_title(r"$\boldsymbol{\ddot{\theta}}$")
    ax0.text(
        s=r"\textbf{Joint 0}",
        x=-0.35,
        y=0.5,
    ...
    ax0.set_ylabel("Acceleration [rad/m2]")
    ax0.get_yaxis().set_label_coords(-0.2, 0.5)
    ax0.set_ylim(y_t_low[0], y_t_max[0])
    ax0.set_xticks(ticks)
    ax0.set_xticklabels(test_labels)
    ...
    ax1.set_ylabel("Acceleration [rad/m2]")
    ax1.get_yaxis().set_label_coords(-0.2, 0.5)
    ax1.set_ylim(y_t_low[1], y_t_max[1])
    ax1.set_xticks(ticks)
    ax1.set_xticklabels(test_labels)
    ...
    # Plot Ground Truth Torque:
    ax0.plot(test_ddq[:, 0], color="k")
    ax1.plot(test_ddq[:, 1], color="k")

    # Plot DeLaN Torque:
    ax0.plot(delan_ddq[:, 0], color=color_i[0], alpha=plot_alpha)
    ax1.plot(delan_ddq[:, 1], color=color_i[0], alpha=plot_alpha)
